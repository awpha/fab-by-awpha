...
?>			<script>
				...
				(function($) {
					...
					jQuery(document).ready(function($) {
						// Initialize color picker on class "js-color-picker"
						$(".wp-color-picker").wpColorPicker();
					});
					// Show/Hide inputs based on selected option value
					jQuery(document).ready(function($) {
						// onLoad
						$(".js-show-selected select").each(function() {
							showSelectedOption($(this));
						});
					
						// onChange
						$(".js-show-selected select").on("change", function() {
							showSelectedOption($(this));
						});
						...
						function showSelectedOption($select) {
							...
							// Hide all classes which start with selected id
							$("[class^=\'" + selectId + "\']").addClass("hidden");
					
							// Get selected value
							var selectedValue = $select.val();
					
							// Show selected value class
							$("[class^=\'" + selectedValue + "\']").removeClass("hidden");
						}
					});
				})(jQuery);
			</script>
		...
		switch ( $field['type'] ) {
			...
			case 'color':
				printf(
					...
					!empty( $field['class'] ) ? "class='wp-color-picker {$field['class']}'" : 'class="wp-color-picker"',	// class
					$field['id'],																		// id
					"{$page_key}[{$field['id']}]",														// name
					!empty( $field['placeholder'] ) ? 'placeholder="'.__($field['placeholder'],'text-domain').'"' : '',		// placeholder
					$field['title_attr'],																// title
					'text',																				// type set to text to avoid native color picker
					$field['value'],																	// value
					!empty( $attributes ) ? implode( ' ', $attributes ) : '',							// additional attributes
					!empty( $field['text'] ) ? '<p class="help">'.__($field['text'],'text-domain').'</p>' : ''				// text
				);
